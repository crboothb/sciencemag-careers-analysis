...
def tag_incidence(
    df, lifespan=True, id_col_tag=False, quantile=0.8, binary=True, dict_return=False
):
    df = seperate_tags(df)
    ...
    for i in df.index.values:
        ...
        if w_tag in i_dict.keys():
            ...
            if lifespan is True:
                ...
                i_dict[w_tag][w_author] = 1
        if lifespan is True:
            i_dict[w_tag]["overall"]["span"].append(
                [df.loc[i, "date_seq"], df.loc[i, "month_seq"]]
            )
    if id_col_tag is True:
...
def id_column_tags(i_dict, quantile=0.8, binary=True):
    col_tag = {}
...
def inc_per_year(i_dict, columns="no"):
    ...
    for key in [key for key in i_dict.keys()]:
        # print(key)
        if (
            i_dict[key]["overall"]["col_tag"] == columns
        ):  # filter out tags that are probs identifying columns
