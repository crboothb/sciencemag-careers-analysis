...
thing = "hedges" # choose: "first", "second", "modal", "hedges","boosters"

if(thing == "first"){
  feature = "frac1"
  med_feature = "med_frac1"
  tags_df = pronouns_tags
  tags_med_df = pronoun_tags_med.short
}
if(thing=="second"){
  feature = "frac2"
  med_feature = "med_frac2"
  tags_df = pronouns_tags
  tags_med_df = pronoun_tags_med.short
}
if(thing=="modal"){
  feature = "frac"
  med_feature = "med_frac"
  tags_df = modal_tags
  tags_med_df = modal_tags_med
}
if(thing=="hedges"){
  feature = "frac_h"
  med_feature = "med_frac_h"
  tags_df = hedges_tags
  tags_med_df = hedges_tags_med.short
}
if(thing=="boosters"){
  feature = "frac_b"
  med_feature = "med_frac_b"
  tags_df = hedges_tags
  tags_med_df = hedges_tags_med.short
}
...
overall_med = median(tags_df[[feature]])

for(i in seq(1,16,1)){
  # print(i)
  # print(pronoun_tags_med.short$Group.1[i])
  samp = nrow(subset(tags_df, level_0==tags_med_df$Group.1[i]))
  print(samp)
  tag_median = tags_med_df[[med_feature]][i]
  test.stat = tag_median-overall_med
  # print(test.stat)
  
  perms = jackknife.permute(x = tags_df[[med_feature]], samp_size=samp, nperm = n, overall = overall_med)
  #print("test")
  ...
  #   ggtitle(tags_df$Group.1[i])
  # print(plot)
  
}

tests.df = data.frame("tags"=tags_med_df$Group.1,"pval"=pvals, "test_statistic"=tests, "tag_median"=tag_medians)

write.csv(tests.df,paste("../../data/bargraph_data/",thing,"_sig_test.csv", sep=""))
# write.csv(tests.df,paste(thing,"_sig_test.csv", sep=""))
